...
final class MembresController extends AbstractController
{
    ...
    UserPasswordHasherInterface $unPasswordHasher): Response
    {
        $membre = new Membre();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $membre->setPassword($unPasswordHasher->hashPassword($membre,
            $passwordSaisi));
    ...
    public function edit(
        ...
        UserPasswordHasherInterface $unPasswordHasher
    ): Response {
        $form = $this->createForm(MembreType::class, $membre);
        $form->handleRequest($request);
